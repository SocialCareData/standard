...
<nav class="category-filter">
  <a class="category" href="{{ page.permalink }}">All</a>{% for cat in sorted_categories %}<a class="category" href="{{ page.permalink }}?category={{ cat | url_encode }}">{{ cat }}</a>{% endfor %}
</nav>
{% endunless %}
...
{% for post in site.posts %}
  <li data-categories="{{ post.categories | join: ',' }}">
    ...
    {% if post.categories.size > 0 %}
      <span class="post-categories">{% for cat in post.categories %}<a class="category" href="{{ page.permalink }}?category={{ cat | url_encode }}">{{ cat }}</a>{% unless forloop.last %} {% endunless %}{% endfor %}</span>
    {% endif %}
  </li>
{% endfor %}
</ul>
