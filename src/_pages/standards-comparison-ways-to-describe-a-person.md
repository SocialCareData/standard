...
| **given** | **Name.givenNames** <br><br> Cardinality: 1..* (in a 1..1 Name Object) <br><br> Definition: First and any middle names. | **given** | **HumanName.given** | **Name → Formal Name → Given Name** <br><br> Middle names captured in Formal Name → Middle Name. | n/a |
| | **Name.preferredNames** <br><br> Cardinality: 0..* (in a 1..1 Name) <br><br> Definition: Any preferred given or middle names used by the person. | | | **Name → Informal Name → Preferred Name** | Supports matching where a person is commonly known by a name other than their legal first name (e.g. Joe for Joseph). Also present in PDS. |
...
| **birthDate** | **DateOfBirth.date** <br><br> Cardinality: 0..1 (in a 1..1 DateOfBirth Object) <br><br> Definition: ISO8601 formatted date of birth (YYYY-MM-DD). | **birthdate** | **birthDate** | **Person Event → Birth → Date of Birth** | GET specifies `format: date`; ISO8601 is assumed. |
...
| | **Deceased.deceasedStatus** <br><br> Cardinality: 1..1 (in a 1..1 Deceased Object) <br><br> Definition: True / False for if person is deceased. | | **deceasedBoolean** | Implied by the presence of a **Person Event → Death** | Required for statutory data collection in adult social care, and used to filter deceased individuals from service allocation. Also relevant as a search filter. |
| | **Deceased.date** <br><br> Cardinality: 0..1 (in a 1..* Deceased Object) <br><br> Definition: ISO8601 formatted date of death (YYYY-MM-DD). | **death-date** | **deceasedDatetime** | **Person Event → Death → Date of Death** | Date of death is not always known even where deceased status is. |
...
| **gender** | **Gender** <br><br> Cardinality: 1..1 <br><br> Definition: The person's stated gender. This information does not pertain to biological sex. Uses NHS PERSON_STATED_GENDER_CODE. | **gender** | **gender** | **Biological Attributes → Sex and Gender** | Gender as stated by the individual. An important search parameter. |
